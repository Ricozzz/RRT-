...
RRT::RRT(ros::NodeHandle &nh): nh_(nh), gen((std::random_device())()) {
    ...
    path_pub_ = nh_.advertise<visualization_msgs::Marker>("/whole_path_point", 0);
	...
	free_point.color.b = 0.0;
      ...
      //visualization sampled points
	whole_path.header.frame_id = "map";
	whole_path.ns = "whole_path_vis";
	whole_path.type = visualization_msgs::Marker::LINE_LIST;//SPHERE;
	whole_path.action = visualization_msgs::Marker::ADD;
	//tree_node_point.pose.position.z = 0;
	//tree_node_point.pose.orientation.x = 0.0;	
	//tree_node_point.pose.orientation.y = 0.0;	
	//tree_node_point.pose.orientation.z = 0.0;	
	whole_path.pose.orientation.w = 1.0;	
	whole_path.scale.x = 0.05;	
	//free_point.scale.y = 0.1;	
	//free_point.scale.z = 0.1;	
	whole_path.color.a = 1.0;	
	whole_path.color.r = 1.0;	
	whole_path.color.g = 1.0;
	whole_path.color.b = 0.0;
    ...
    std::ifstream fin("/home/zan/zhenzan_ws/src/rrt/src/mymap.csv");
    std::string line;
    ...
    std::ifstream fin2("/home/zan/zhenzan_ws/src/rrt/src/wp_mymap.csv");
    std::string line2;
...
*/
    ...
    for(int i=0; i<distant.size(); i++){
	scan_ << cos(i*angle) * (distant[i]-0.01), sin(i*angle) * (distant[i]-0.01);//0.3
	...
	if((row <= 115 && row >= 4) && (col >= 4 && col <= 115)){
		for(int m=0;m<4;m++){
			for(int n=0;n<8;n++){
				...
				submap[119-(col-4+m)][119-(row-4+n)] = 0.0;
...
///////////////////////////////////////////////////////////////////////////////////////
	...
	float closest_velocity = 0;
	float goalpoint_velocity = 0;
	...
	for(int i=0;i<waypoint.size();i++){		   
		if(pow(waypoint[search][0]-curr_x,2)+pow(waypoint[search][1]-curr_y,2) == pow(LOOK_AHEAD,2)){
			...
			x = waypoint[search][0];
			y = waypoint[search][1];
			goalpoint_velocity = waypoint[search][2];
			break;
		}
		...
		if((pow(waypoint[search][0]-curr_x,2)+pow(waypoint[search][1]-curr_y,2) <= pow(LOOK_AHEAD,2)) && (pow(waypoint[search+1][0]-curr_x,2)+pow(waypoint[search+1][1]-curr_y,2) >= pow(LOOK_AHEAD,2))){
			...
			goalpoint_velocity = (waypoint[search][2] + waypoint[search+1][2])/2;
			break;
	...
	//the closest_velocity, goalpoint_velocity
	ROS_INFO("the closest velocity %f and goal velocity %f", closest_velocity, goalpoint_velocity);
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	double first_node_weigh = 0.9;
	new_node.parent = 0;
	new_node.x = first_node_weigh*root_.x + (1-first_node_weigh)*goal_submap_x;
	new_node.y = first_node_weigh*root_.y + (1-first_node_weigh)*goal_submap_y;
...
*/
    ...
    for(int k=0; k<iteration_; k++){
	ROS_INFO("now we get the xrand");
	sampled_point = sample(goal_submap_x, goal_submap_y);
	ROS_INFO("sampled sample.x %f, sample.y %f", sampled_point[0], sampled_point[1]);
	nearest_node = nearest(tree, sampled_point);
	ROS_INFO("nearest node index %d", nearest_node);
	ROS_INFO("the nearest x %f, y %f", tree[nearest_node].x, tree[nearest_node].y);
	...
	if(!check_collision(tree[nearest_node], new_node)){
		...
		tree.push_back(new_node);
		//ROS_INFO("add the new node %f, %f to tree", new_node.x, new_node.y);
...
//////////////////
//find the goal point
float find_min_cost = 1000000;
int find_min = 0;
for(int i=0; i<tree.size(); i++){
	float find_dist = sqrt(pow(tree[i].x-goal_submap_x, 2) + pow(tree[i].y-goal_submap_y, 2));
	if(find_dist < threshold_ && cost(tree, tree[i])<find_min_cost){
		find_min_cost = cost(tree, tree[i]);
		find_min = i;
	}
}
new_node = tree[find_min];
ROS_INFO("the final goal point is x %f, y %f", new_node.x, new_node.y);
...
/////////////////////////////////////////////////////////////
	...
	//ROS_INFO("ttttttttttttttttttt %f, %f", temp_plan[0].pose.position.x,temp_plan[0].pose.position.y);
...
////////////////////////////////////////
	...
	whole_path.points.clear();

    for(int i=0; i< tree.size(); i++){
	if(!tree[i].is_root){
		whole_path.id = 5;
		whole_path.header.stamp = ros::Time::now();
		geometry_msgs::Point p;
		p.x = curr_.x + (tree[i].x - 60)*map_resolution_;
		p.y = curr_.y - (tree[i].y - 60)*map_resolution_;
		p.z = 0;
		whole_path.points.push_back(p);
		p.x = curr_.x + (tree[tree[i].parent].x - 60)*map_resolution_;
		p.y = curr_.y - (tree[tree[i].parent].y - 60)*map_resolution_;
		whole_path.points.push_back(p);
	}
    }
    path_pub_.publish(whole_path);
...
//////////////////////////////////////////////////////////////////////////////////////
	//////////////////////chooooooose which point to chase/////////////////
/////////////////////////////////////////////////////////////////////////////////////

    if(found_path.size()>4){
    x_m << (found_path[found_path.size()-3].x-60)/20 + curr_x, -(found_path[found_path.size()-3].y-60)/20 + curr_y;//7?
    }else{
    ...
    //this is the chosen point in vehicle frame
...
////////////////////////////////velocity//////////////////
	...
	//	vel = velocity*0.75;
	//}
	//vel = (closest_velocity + goalpoint_velocity)/2;
...
std::vector<double> RRT::sample(int goal_submap_x, int goal_submap_y) {
    ...
    std::mt19937 gen(rd());
    std::uniform_real_distribution<double> x_dist(goal_submap_x - 40, goal_submap_x + 40);//30
    std::uniform_real_distribution<double> y_dist(goal_submap_y - 40, goal_submap_y + 40);
