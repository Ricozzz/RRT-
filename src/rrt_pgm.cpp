...
RRT::RRT(ros::NodeHandle &nh): nh_(nh), gen((std::random_device())()) {
    ...
    nh_.getParam("map_sizex_", map_sizex_);
    nh_.getParam("map_sizey_", map_sizey_);
    nh_.getParam("map_resolution_", map_resolution_);
    nh_.getParam("map_origin_x_", map_origin_x_);
    nh_.getParam("map_origin_y_", map_origin_y_);
    ...
    for(int i=0; i<map_sizex_; i++){
	for(for j=0; j<map_sizey_; j++){
	    map_info_[i][j] = 0;
...
void RRT::pf_callback(const nav_msgs::Odometry::ConstPtr &pose_msg) {
    ...
    rrt_nodes_->x = pose_msg->pose.pose.position.x;
    rrt_nodes_->y = pose_msg->pose.pose.position.y;
...
bool RRT::is_goal(Node &latest_added_node, double goal_x, double goal_y) {
    ...
    if(sqrt(pow(latest_added_node->x - goal_x, 2) + pow(latest_added_node->y - goal_y, 2)) * map_resolution_ < threshold_){
	close_enougt = true;
    }
    return close_enough;
}
